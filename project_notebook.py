...
def create_fScore(self):
    ...
    fScore[self.start] = self.start + self.heuristic_cost_estimate(self.start)
...
def get_current_node(self):
    ...
    curr_node_fscore = list(self.openSet)[0]
    
    for node in self.openSet:
        if self.fScore.get(node, np.inf) < self.fScore.get(curr_node_fscore, np.inf): curr_node_fscore = node
    
    return curr_node_fscore
...
def get_gScore(self, node):
    ...
    return self.gScore.get(node, np.inf)
...
# ---
# _Some__ admissible heuristic are consistent.
# 
# *CHOOSE ONE*
#     - All
#     - Some
...
# ---
# _All__ Consistent heuristic are admissible.
# 
# *CHOOSE ONE*
#     - All
#     - Some
